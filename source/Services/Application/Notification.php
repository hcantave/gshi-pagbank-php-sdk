...
use PagSeguro\Parsers\Authorization\Notification\Request;
...
class Notification
{
    ...
    public static function check(Credentials $credentials)
    {
        ...
        try {
            ...
            $http = new Http();
            Logger::info(sprintf("GET: %s", self::request($data)), ['service' => 'Application.Notification']);
            $http->get(
                self::request($data),
                20,
    ...
    {
        return $data->buildNotificationAuthorizationRequestUrl() . "/" .
